#define WEDGE_HANDS
/***************************************************************************/
void updateAnalogClock(int hours, int minutes, int seconds, time_t workTime)
/***************************************************************************/
{
  startMillis = millis();
  ...
  for (int ordinal = 0; ordinal < 12; ordinal++) {
    angle = ordinal * 30 * 0.0174533;  // Calculate in degrees then change to radians.

    myX2 = xCenter + (sin(angle) * (radius - 20));
    myY2 = yCenter + (cos(angle) * (radius - 20));
    myX2 -= 10; myY2 -= 10;
  ...
  if (showShortTime) {
    strftime(charWork, sizeof(charWork), "%H:%M", localtime(&workTime));  // Hour and minute only.
    ofr.setCursor(tft.width() / 2 - 39, tft.height()*.64); ofr.printf(charWork);
  }
  if (showLongTime) {
    strftime(charWork, sizeof(charWork), "%H:%M:%S", localtime(&workTime));  // Hour, minute & second.
  ...
  clockSprite.drawWedgeLine(myX2, myY2, xCenter, yCenter, 1, 8, TFT_WHITE);
  ...
  // The widths coorespond to the x,y pairs.  myX2/Y2 goes with 2, xCenter/yCenter goes with 10.
#if defined WEDGE_HANDS
  clockSprite.drawWedgeLine(myX2, myY2, xCenter, yCenter, 1, 8, TFT_RED);
#else  // Wire frame hands, coloron one side, alternating across center
  ...
  if (showSecondHand) {  // Show second hand on 2 and 4
    ...
    // The widths coorespond to the x,y pairs.  myX2/Y2 goes with 2, xCenter/yCenter goes with 10.
#if defined WEDGE_HANDS
    clockSprite.drawWedgeLine(myX2, myY2, xCenter, yCenter, 1, 6, TFT_BLUE);
#else  // Wire frame hands, coloron one side, alternating across center
    // Locate backside of hand (across the center).
  ...
  if ((showShortTime || showLongTime) && PHP_Rate > 0.) {  // Yeah, the extra parens is needed!
    dtostrf(PHP_Rate, 2, 2, charWork);  // Because OFR with all its fancy coding is not capable!
    ofr.setCursor(tft.width() / 2, tft.height()*.28); ofr.cprintf(charWork);
  }
...
/***************************************************************************/
{
  ...
  strftime(charWork, sizeof(charWork), "%a", localtime(&workTime));  // name Day
  ...
  if (timeinfo->tm_mon == 5 || timeinfo->tm_mon == 6)
    strftime(charWork, sizeof(charWork), "%B", localtime(&workTime));  // Short month name
  else
    strftime(charWork, sizeof(charWork), "%b", localtime(&workTime));  // Short month name
  ...
  strftime(charWork, sizeof(charWork), "%d", localtime(&workTime));  // Numeric day of month
  ofr.setCursor(0, tft.height() - 26); ofr.printf(charWork);

  strftime(charWork, sizeof(charWork), "%Y", localtime(&workTime));  // Full (4 digits) year
